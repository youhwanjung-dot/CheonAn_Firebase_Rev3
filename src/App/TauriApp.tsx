...
const migrateData = (localData: AppData, resourceData: AppData): AppData => {
    ...
    const mergedData: AppData = {
        ...
        dbVersion: resourceData.dbVersion,
    };
    const mergeCollection = <T extends { id: string } | string>(localList: T[] = [], newList: T[] = [], key?: keyof T) => {
        if (!key) {
            const localSet = new Set(localList);
            const newItems = newList.filter(item => !localSet.has(item));
            return [...localList, ...newItems];
        }
        const localSet = new Set(localList.map(item => (item as any)[key]));
        const newItems = newList.filter(item => !localSet.has((item as any)[key]));
        return [...localList, ...newItems];
    };
...
const TauriApp = () => {
    ...
    const [dbVersion, setDbVersion] = useState<string | undefined>();
    ...
    useEffect(() => {
        if (!isDataLoaded || !dbVersion) return;
    ...
    }, [inventory, transactions, users, sites, locations, dbVersion, isDataLoaded]);
    ...
    const handleTransactionUpdate = (oldTx: InventoryTransaction, updates: Partial<InventoryTransaction>) => {
        ...
        setInventory(prev => prev.map(item => item.id === oldTx.itemId ? { ...item, currentStock: item.currentStock + stockDifference } : item));
    ...
    const handleTransactionDelete = (txToDelete: InventoryTransaction) => {
        ...
        setInventory(prev => prev.map(item => item.id === txToDelete.itemId ? { ...item, currentStock: item.currentStock + stockImpact } : item));
    ...
    const transactionItem = useMemo(() => inventory.find(i => i.id === transactionItemId), [inventory, transactionItemId]);
    ...
    return (
        <div className="flex h-screen bg-slate-100 dark:bg-slate-950 transition-colors overflow-hidden font-sans">
            ...
            {showTransactionModal && transactionItem && <TransactionModal item={transactionItem} transactions={transactions} currentUser={user} users={users} onSave={handleTransactionSave} onUpdate={handleTransactionUpdate} onDelete={handleTransactionDelete} onClearHistory={() => setShowClearHistoryConfirm(true)} onClose={() => setShowTransactionModal(false)} />}
            ...
            <Suspense fallback={null}>
                ...
                {showMasterDataManager && <MasterDataManagerModal inventory={inventory} onUpdateInventory={setInventory} onClose={() => setShowMasterDataManager(false)} />}
